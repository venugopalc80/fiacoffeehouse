...
import { CalendarDays, ChevronRight, Flower2, Gift, Instagram, MapPin, Menu as MenuIcon, Phone, X } from 'lucide-react';
import './styles.css';
...
function App() {
  ...
  return <div className="app">
    ...
    {modal === 'floral' && <Modal eyebrow="LEIA FLORALS" title="Tell us about your flowers." onClose={closeModal}><form onSubmit={e => handleForm(e, 'Thanks. Your floral enquiry has been recorded.')}><input required placeholder="Name" /><input required type="email" placeholder="Email" /><select required defaultValue=""><option value="" disabled>What are you looking for?</option><option>Bouquet</option><option>Event flowers</option><option>Wedding flowers</option><option>Gift flowers</option><option>Seasonal collection</option><option>Other</option></select><input type="date" /><textarea placeholder="Tell us what you have in mind" /><button className="primary">Send Floral Enquiry</button></form></Modal>}
    {modal === 'event' && <Modal eyebrow="EVENTS & PRIVATE HIRE" title="Make your occasion a FIA moment." onClose={closeModal}><form onSubmit={e => handleForm(e, 'Thanks. Your event enquiry has been recorded.')}><input required placeholder="Name" /><input required type="email" placeholder="Email" /><input required type="tel" placeholder="Phone" /><select required defaultValue=""><option value="" disabled>Event type</option><option>Birthday</option><option>Baby shower</option><option>Brunch gathering</option><option>Corporate event</option><option>Private hire</option><option>Other</option></select><div className="two"><input type="date" /><input min="2" max="100" type="number" placeholder="Guests" /></div><textarea placeholder="Tell us about your occasion" /><button className="primary">Send Event Enquiry</button></form></Modal>}
    {modal === 'gift' && <Modal eyebrow="GIVE FIA" title="Gift a FIA moment." onClose={closeModal}><form onSubmit={e => handleForm(e, 'Thanks. We’ll contact you about the FIA gift card request.')}><input required placeholder="Your name" /><input required type="email" placeholder="Your email" /><input required placeholder="Recipient name" /><select required defaultValue=""><option value="" disabled>Gift value</option><option>£20</option><option>£30</option><option>£50</option><option>£100</option><option>Other</option></select><textarea placeholder="Gift message (optional)" /><button className="primary">Request Gift Card</button></form></Modal>}
    {modal === 'gallery' && <Modal eyebrow="MOMENTS AT FIA" title="A glimpse of the FIA experience." onClose={closeModal}><div className="gallery-modal-grid">{gallery.slice(0, 6).map(([className, label]) => <div key={className} className={`g ${className}`}><span>{label}</span></div>)}</div><a className="insta" href="https://www.instagram.com/fiacoffeehouse/" target="_blank" rel="noreferrer"><Instagram size={18} /> See more on Instagram</a></Modal>}
  </div>;
}
