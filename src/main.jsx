...
import { CalendarDays, ChevronRight, Gift, MapPin, Menu as MenuIcon, Phone, X } from 'lucide-react';
import './styles.css';
...
const gallery = [
  ['coffee', 'Coffee'], ['brunch', 'Brunch'], ['interior', 'Interior'],
  ['coffee2', 'Coffee'], ['brunch2', 'Brunch'], ['interior2', 'Interior']
];
...
function App() {
  ...
  const nav = ['Home', 'Menu', 'Events', 'Gallery', 'About', 'Visit Us'];
  ...
  return <div className="app">
    ...
    <section id="home" className="hero">
      <div className="hero-overlay">
        <p className="eyebrow">COFFEEHOUSE · BRUNCH · GOOD COMPANY</p>
        <h1>Good Coffee.<br />Brighter Days.</h1>
        <p className="lead">Speciality coffee and seasonal brunch, thoughtfully made in the heart of Preston.</p>
    ...
    <section className="intro">
      ...
      <p>At FIA, speciality coffee meets fresh brunch, creating a warm space where good food and great ideas come together.</p>
      <div className="pillars two-pillars">
    ...
    <section id="menu" className="menu-section">
      ...
      <div className="menu-grid">{menuFilter === 'Brunch' ? brunch.slice(0, 6).map(group => <article className="menu-card" key={group.title}><h3>{group.title}</h3>{group.items.map(item => <div className="row" key={item}><span>{item}</span><span>›</span></div>)}</article>) : filteredMenu.map(group => <article className="menu-card" key={group.title}><h3>{group.title}</h3>{group.items.map(item => <div className="row" key={item}><span>{item}</span><span>›</span></div>)}</article>)}</div>
    ...
    <section className="seasonal"><div className="section-head"><p className="eyebrow">WHAT'S HAPPENING AT FIA</p><h2>Seasonal moments, thoughtfully made.</h2></div><div className="seasonal-grid two-seasonal">{seasonal.map(item => <article key={item.title}><div className="seasonal-image" style={{ backgroundImage: `url(${item.image})` }} /><div><p className="eyebrow">FIA SEASONAL</p><h3>{item.title}</h3><p>{item.text}</p><ChevronRight size={18} /></div></article>)}</div></section>

    <section id="gallery" className="gallery"><div className="section-head"><p className="eyebrow">MOMENTS AT FIA</p><h2>Coffee. Brunch. People.</h2></div><div className="gallery-grid">{gallery.map(([className, label], index) => <button className={`g ${className}`} key={index} aria-label={`Open ${label} image`} onClick={() => setModal('gallery')}><span>{label}</span></button>)}</div><a className="insta" href="https://www.instagram.com/fiacoffeehouse/" target="_blank" rel="noreferrer"><InstagramIcon size={18} /> Follow on Instagram</a></section>

    <section id="about" className="about"><div><p className="eyebrow">OUR STORY</p><h2>A space for good food, great company and beautiful moments.</h2><p>FIA Coffeehouse brings together a simple idea: speciality coffee and seasonal brunch in one welcoming place.</p><p>We’re more than a café; we’re a place to slow down, connect and enjoy the little things.</p><button className="outline" onClick={() => go('visit-us')}>Plan Your Visit</button></div><div className="about-image" /></section>
    ...
    <section className="newsletter"><Gift /><div><p className="eyebrow">JOIN OUR FAMILY</p><h2>Seasonal menus, new dishes and FIA moments.</h2><form onSubmit={e => { e.preventDefault(); setNewsletter('Thanks, you’re on the FIA list.'); e.currentTarget.reset(); }}><input required type="email" placeholder="Your email address" aria-label="Email address" /><button className="primary">Subscribe</button></form>{newsletter && <p className="success">{newsletter}</p>}</div></section>
    ...
    {modal === 'gallery' && <Modal eyebrow="MOMENTS AT FIA" title="Follow FIA on Instagram." onClose={closeModal}><p>See the latest coffee, brunch and café moments from FIA Coffeehouse.</p><a className="primary" href="https://www.instagram.com/fiacoffeehouse/" target="_blank" rel="noreferrer"><InstagramIcon /> Open Instagram</a></Modal>}
  </div>;
}
